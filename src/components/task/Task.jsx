		...
		<>
			<div id={`task-${task._id}`} style={{
				...
				<div className="flex justify-between items-center mb-4">
					...
					<div onClick={_ => task.status === 'done' ? markUncompleted(task._id) : markCompleted(task._id)} className="cursor-pointer flex items-center space-x-2 border-green-600 rounded-md p-2 bg-green-200">
						<input
							type="checkbox"
							id={`task-${task._id}`}
							className="cursor-pointer form-checkbox h-4 w-4 text-indigo-900 rounded"
							checked={task.status === 'done'}
							onChange={_=>{}}
						/>
						<label htmlFor={`task-${task._id}`} className="cursor-pointer text-indio-800 text-sm">
							{task.status === "done" ? "Mark Uncompleted" : "Mark Completed"}
						</label>
					</div>
				</div>
